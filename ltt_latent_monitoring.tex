...
Here we can use fixed sequence testing \citep{angelopoulos_learn_2022} on \( \lambda \) ordered from largest to smallest.

\subsection{Combined Safety and Budget Guarantees}
\label{sec:combined_safety_and_budget_guarantee}
...
\[ 
    \mathcal{T}_{\mathbf{\bm{\lambda}}}(z)= 
    \begin{cases}
        0, & s_l(z)_0 > \lambda_0, \\
        1, & s_l(z)_1 > \lambda_1, \\
        \text{arg max}\{s_i(z)\}, & \text{else}.
...
\[
    R_{cost}(\mathcal{T}_{\bm{\lambda}}) = \mathbb{P}(s_l(z)_0 < \lambda_0 \wedge s_l(z)_1 < \lambda_1).
\]
\[
    \hat{R}_{cost} = \frac{
        \sum_{i=1}^n \indicator\{ s_l(z)_0 < \lambda_0 \wedge s_l(z)_1 < \lambda_1 \}
    }{
        n
    }.
\]
...
\paragraph{Cascaded 2D testing}
This leaves us with a 2D grid of parameters to search over, for which we can use cascaded 2D testing or fallback SGT testing \citep{angelopoulos_learn_2022}.
While we want both risks to be controlled simultaneously, we aren't really interested in \emph{minimising} \(R_{cost}\), only \emph{satisfying} it. 
However, conditional on the \(R_{cost}\) being satisfied
We display decreasingly conservative \(\lambda_0\) candidates across the columns of our grah and decreasingly conservative \(\lambda_1\) candidates across the rows, then proceed by testing row-wise (i.e., for fixed \(\lambda_1\)), find the least conservative \(\lambda_0\). 
If we reach the end of a row without failing to reject any hypothesis, we propagate the row's budget to the next row and look for an improved \(R_{cost}\). 
Finally, we return the pair with the least conservative \( \lambda_0 \).  

\paragraph{Pareto Testing}




\section{Thresholding with Epistemic uncertainty}
We design the latent probe \(s_l\) as an evidential classifier to explicitly model epistemic uncertainty. 
Instead of a standard softmax output, \(s_l\) employs a non-negative activation function (e.g., Softplus) 
to predict an evidence vector \(\mathbf{e} = [e_1, e_2]^\top\) for the binary classes. 
This evidence parameterizes a Dirichlet distribution \(\text{Dir}(\boldsymbol{p} \mid \boldsymbol{\alpha})\) 
via the transformation \(\alpha_k = e_k + 1\). 
The expected class probability is computed as \(\hat{p}_k = \alpha_k / S\), and the associated 
epistemic uncertainty is derived as \(u = K / S\), where \(K=2\) (binary case), \(S = \sum_{k=1}^K \alpha_k\) is the Dirichlet strength.


\paragraph{Symmetric Thresholding}
\[
    \mathcal{T}_{\mathbf{\lambda}}(z)= 
    \begin{cases}
        \text{arg max}\{s_i(z)\}, & u \geq \lambda_u, \\
        0, & \hat{p}_0 > \lambda_p \text{ and } u < \lambda_u, \\
        1, & \hat{p}_1 > 1 - \lambda_p \text{ and } u < \lambda_u, \\
    \end{cases}
\]

\[
    R_{cost}(\mathcal{T}_{\bm{\lambda}}) = \mathbb{P}(u \geq \lambda_u).
\]
We can use analogous safety risks to section \ref{sec:combined_safety_and_budget_guarantee}
\[
    R_{safety}(\mathcal{T}_{\bm{\lambda}}) = \mathbb{P}(Y=1 \mid \hat{p}_0 > \lambda_p, u < \lambda_u).
\]
...
\[
    \mathcal{T}_{\mathbf{\lambda}}(z)= 
    \begin{cases}
        0, & \hat{p}_0 > 0.5 \land u < \lambda_0, \\
        1, & \hat{p}_1 > 0.5 \land u < \lambda_1, \\
        \text{delegate} & \text{else}
    \end{cases}
\]

\[
    R_{cost}(\mathcal{T}_{\bm{\lambda}}) = \mathbb{P}\bigl(
        (u \geq \lambda_0 \land \hat{p}_0 > 0.5) 
        \land 
        (u \geq \lambda_1 \land \hat{p}_1 > 0.5)
        \bigr).
\]

\[
    R_{safety}(\mathcal{T}_{\bm{\lambda}}) = \mathbb{P}(Y=1 \mid \hat{p}_0 > 0.5, u < \lambda_0).
\]


\input{extensions.tex}

\input{SGT_with_MILP.tex}
